...
模板编译器会对模板进行静态分析，以识别出**静态节点**，并为这些节点添加 `Patch Flag`。`Patch Flag` 是一个标志位，用于**告诉运行时渲染器如何处理该节点**，以提高渲染性能。

Patch Flag 的值是一个 32 位的二进制表示，每一位代表一个特定的操作或标识。以下是 Patch Flag 的一些常用标识：
