...
+ `T extends [infer F, ...infer R]`是一个泛型约束语法，用于描述一个泛型类型T必须是一个由至少一个类型F和零个或多个类型R组成的数组。
+ `infer`关键字用于在泛型约束中声明一个临时的类型变量，用于类型推断
+ `infer F`表示需要推断的第一个元素的类型，而`...infer R`表示需要推断的剩余元素的类型。

例如，对于类型`[string, number, boolean]`，其中`string`是F，而`[number, boolean]`是R。
~~~ts
...
`keyof any`表示任意类型的所有属性键的联合类型。因此，K必须是这个**联合类型的子集**。
